...
<head>
    ...
    <title>User Management | LeaveWork</title>
    <link rel="icon" type="image/png" href="{{ asset('assets/leavework_logo.png') }}">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.0/font/bootstrap-icons.css">
    <style>
        :root { --primary: #19183B; --secondary: #708993; --accent: #A1C2BD; --light: #E7F2EF; }
        body { font-family: 'Segoe UI', system-ui; background: var(--light); color: var(--primary); display: flex; min-height: 100vh; }
        #sidebar { width: 250px; background: #fff; border-end: 1px solid var(--accent); position: fixed; min-height: 100vh; z-index: 1000; }
        ...
        .page-header { background: white; padding: 1.5rem 2rem; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); margin-bottom: 2rem; border-left: 4px solid var(--primary); }
        .table-container { background: white; border-radius: 12px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); overflow: hidden; }
        .table-header { background: var(--primary); color: white; padding: 1rem 1.5rem; }
        .table th { font-weight: 600; text-transform: uppercase; font-size: 0.85rem; color: var(--primary); border-bottom: 2px solid var(--accent); padding: 1rem 1.5rem; }
        .table td { padding: 1rem 1.5rem; border-bottom: 1px solid #eee; }
        .table tr:hover { background: rgba(161, 194, 189, 0.05); }
        .badge-role { font-weight: 500; padding: 0.4rem 0.8rem; border-radius: 20px; }
        .badge-role.admin { background: #d1ecf1; color: #0c5460; }
        .badge-role.employee { background: #d4edda; color: #155724; }
        .badge-role.superuser { background: #f8d7da; color: #721c24; }
        .sortable { cursor: pointer; transition: color 0.2s; position: relative; user-select: none; }
        .sortable:hover { color: var(--secondary); }
        .sort-icon { font-size: 0.75rem; margin-left: 0.25rem; opacity: 0.7; transition: opacity 0.2s; }
        .sortable.active .sort-icon { opacity: 1; }
        .sortable.asc .sort-icon i::before { content: "\F128"; }
        .sortable.desc .sort-icon i::before { content: "\F138"; }
        .data-label { font-weight: 500; color: var(--secondary); font-size: 0.9rem; }
        .email-cell { font-size: 0.9rem; color: var(--secondary); }
        @media (max-width: 768px) {
            .content-area { padding: 1rem; }
            .table thead { display: none; }
            .table tr { display: block; margin-bottom: 1rem; border: 1px solid #dee2e6; border-radius: 8px; }
            .table td { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 1rem; }
            .table td:before { content: attr(data-label); font-weight: 600; color: var(--primary); min-width: 120px; }
            .email-cell { font-size: 0.85rem; }
        }
    </style>
</head>
<body>
            ...
            <div class="content-wrapper">
                <div class="page-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="h3 mb-2" style="color: var(--primary);">
                                @if(session('role') === 'admin') Employee List
                                @elseif(session('role') === 'superuser') Administrator List
                                @endif
                            </h1>
                            <p class="text-muted mb-0">
                                @if(session('role') === 'admin') View all employees in your company
                                @elseif(session('role') === 'superuser') View all system administrators
                                @endif
                            </p>
                        </div>
                        <span class="badge bg-light text-dark p-2">
                            <i class="bi bi-people-fill me-1"></i>
                            <span id="userCount">{{ $users->total() }}</span> 
                            @if(session('role') === 'admin') Employees
                            @elseif(session('role') === 'superuser') Administrators
                            @endif
                        </span>
                    </div>
                </div>
                ...
                @if($users->isEmpty())
                    <div class="alert alert-info">
                        @if(session('role') === 'admin') No employees found in your company.
                        @elseif(session('role') === 'superuser') No administrators found.
                        @else You do not have permission to view users.
                        @endif
                    </div>
                @else
                    <div class="table-container">
                        <div class="table-header d-flex justify-content-between align-items-center">
                            <h5 class="mb-0">
                                @if(session('role') === 'admin') Employee List
                                @elseif(session('role') === 'superuser') Admin List
                                @endif
                            </h5>
                        </div>
                                ...
                                <tbody id="userTableBody">
                                    @foreach($users as $user)
                                        <tr>
                                            ...
                                            <td data-label="Full Name"><span class="data-label d-md-none">Full Name: </span>{{ $user->last_name }}, {{ $user->first_name }}@if($user->middle_name){{ strtoupper(substr($user->middle_name, 0, 1)) }}.@endif</td>
            ...
            class TableSorter {
                ...
                sortTable(column) {
                    ...
                    const sortedRows = this.originalRows.slice().sort((a, b) => {
                        ...
                        comparison = aValue.localeCompare(bValue, undefined, { sensitivity: 'base', numeric: true });
