...
class AddUserController extends Controller
{
    ...
    public function createUser(Request $request)
    {
        try {
            ...
            try {
                $resetLink = url('/password/reset'); // Replace with your actual reset link logic
                $fullName = $request->first_name . ' ' . $request->last_name;

                Mail::to($request->email, $fullName)
                    ->send(new \App\Mail\AccountCreatedMail(
                        $request->first_name,
                        $request->email,
                        '123456',
                        $resetLink,
                ...
                $errors[] = "Employee created but failed to send email. Mailer Error: " . $e->getMessage();
            }
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'message' => 'The account has been created successfully. ' .
                        'The default password is <strong>123456</strong>.<br>' .
                        'An email with the account details has been sent to the user.'
                ]);
            }
